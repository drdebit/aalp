...
(defn costing-payload
  "What the student must settle before the year moves on. The obligation
   itself is `simulation/unmatched-costs` -- one implementation, so the
   Guided Year and the simulation cannot drift on what counts as owed."
  [user-id]
  (some-> (simulation/costing-payload user-id)
          (assoc :phase "year1")))

(defn submit-costing!
  "Identify the goods that went out of a recorded sale, then re-offer the
   day. The refusal, when there is one, is the record's own."
  [user-id entry-id batch]
  (let [r (simulation/submit-costing! user-id entry-id batch)]
    (cond-> r
      (:ok? r) (assoc :next (day-payload user-id)))))
...
(defn day-payload
  ...
  (let [pos (get-position user-id)
    ...
    (cond
      ...
      (seq (simulation/unmatched-costs user-id))
